...
export function buildProgramsXml(data: GridApiResponse): string {
  ...
  for (const channel of data.channels) {
    for (const event of channel.events) {
      ...
      if (event.flag?.includes("New")) xml += `    <new />\n`;
      if (event.flag?.includes("Live")) xml += `    <live />\n`;
      if (event.flag?.includes("Premiere")) xml += `    <premiere />\n`;
      if (event.flag?.includes("Finale")) xml += `    <last-chance />\n`;
      if (!event.flag?.includes("New") && !event.flag?.includes("Live")) {
        xml += `    <previously-shown />\n`;
      }
      if (event.tags?.includes("Stereo")) xml += `    <audio type="stereo" />\n`;
      if (event.tags?.includes("CC")) xml += `    <subtitles type="teletext" />\n`;
      if (event.program.season && event.program.episode) {
        const se = `S${event.program.season.padStart(2, "0")}E${event.program.episode.padStart(2, "0")}`;
        xml += `    <episode-num system="onscreen">${escapeXml(se)}</episode-num>\n`;
        xml += `    <episode-num system="common">${escapeXml(se)}</episode-num>\n`;
...
export function buildXmltv(data: GridApiResponse): string {
  ...
  xml += '<tv generator-info-name="gracenote-epg" generator-info-url="https://github.com/andrew867/gracenote-epg">\n';
